...
template <typename T>
[[nodiscard]] static inline auto castVoidPtr(T * p) noexcept {
...
static void VS_CC nlmCreate(
    ...
    d->h2_inv_norm = square(255.0f) / (3.0f * square(d->h) * square(2 * d->s + 1));
    ...
    auto channels_raw = vsapi->mapGetData(in, "channels", 0, &err);
    ...
    VSFilterDependency deps[] = {
        { data->node, data->d == 0 ? rpStrictSpatial : rpGeneral },
        { data->ref_node, data->d == 0 ? rpStrictSpatial : rpGeneral }
    };

    vsapi->createVideoFilter(
        out, "NLMeans", data->vi, nlmGetFrame, nlmFree,
